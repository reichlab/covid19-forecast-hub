...
process_imperial_file <- function(sample_mat, location, timezero, qntls=c(0.01, 0.025, seq(0.05, 0.95, by=0.05), 0.975, 0.99)) {
    ...
    obs_data <- read_csv("../../data-truth/nytimes/truth_nytimes-Cumulative Deaths.csv") %>%
    ...
    ## SB: This only gets you the number of predicted deaths on Saturday.
    ## wherewas what want is the total number of deaths predcited for
    ## this week i.e. Sunday through Saturday.
    ## samples_weekly <- sample_mat[,which_weeks]
    ## Therefore you take sum across rows
    samples_weekly <- apply(sample_mat, 1, sum)
    ## Add the observed number of deaths on Saturday.
    last_but_1_obs_death <- obs_data$value[which(obs_data$location=="US" &
                                                 obs_data$date==last_obs_date - 1)]
    sat_inc_deaths <- last_obs_death - last_but_1_obs_death
    samples_weekly <- samples_weekly + sat_inc_deaths
    ...
    if(is.null(dim(samples_weekly))){
      ...
      ## incident deaths on Saturday i.e. when the
