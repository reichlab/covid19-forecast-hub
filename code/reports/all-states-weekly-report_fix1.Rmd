...
date: "`r Sys.Date()` [Prediction Intervals fixed]"
...
# Background
This report provides a brief summary of the weekly ensemble forecast for `r state_name` from the [COVID-19 Forecast Hub](https://covid19forecasthub.org/). In collaboration with the US CDC, our team aggregates COVID-19 forecasts from dozens of teams around the globe. Typically on Wednesday of each week, a summary of the week's forecasts from the COVID-19 Forecast Hub appear on the [official CDC COVID-19 forecasting page](https://www.cdc.gov/coronavirus/2019-ncov/covid-data/forecasting-us.html).
...
Weekly reports from the COVID-19 Forecast Hub can be found at [this page](https://covid19forecasthub.org/doc/reports/). 
...
inc_hosp_targets <- paste(1:28, "day ahead inc hosp")
...
state_inc_hosp_wk_pi <- dat_hosp %>% 
  filter(fips==state, target == "28 day ahead inc hosp", quantile %in% c(0.025, 0.975)) %>% 
  pull(value) %>% sort() %>% 
  format(big.mark = ",")
...
inc_death_truth  <- load_truth(
                    ...
                    mutate(model = "observed data (JHU)") %>%     #maybe also deleting this line
                    left_join(locs, by=c("fips")) %>%
                    filter(target_end_date %in% seq.Date(as.Date("2020-01-25"), to = last_5_saturdays[5], by="1 week"),
...
inc_death_forecast_p6<-load_forecasts(models=c("COVIDhub-ensemble"),forecast_dates =last_6_mondays,locations =state,  types = c("quantile"),targets = c(inc_death_targets),  verbose = FALSE)
...
inc_hosp_truth <- load_truth(
                    ...
                    rename(fips = location) %>%
                    left_join(locs, by=c("fips")) %>%
                      filter(target_end_date <= last_6_saturdays[6])
...
# submit query for forecasts past 6 weeks-using covidhubutils
inc_hosp_forecast_p6<-load_forecasts(models=c("COVIDhub-ensemble"),forecast_dates =last_6_mondays,locations =state,  types = c("quantile"),targets = paste(1:28, "day ahead inc hosp"),  verbose = FALSE,
       source = "zoltar")%>%
...
The ensemble model predicts that weekly totals of observed cases in each of the next four weeks will be between `r state_inc_case_range[1]` and `r state_inc_case_range[2]` cases each week. However for the week ending `r saturday_4_wk_ahead_txt`, the ensemble shows substantial uncertainty, with observed cases between `r state_inc_case_wk_pi_round[1]` and `r state_inc_case_wk_pi_round[2]` deemed possible (95% prediction interval:  `r state_inc_case_wk_pi[1]` -  `r state_inc_case_wk_pi[2]`).
...
# submit query for forecasts past 6 weeks-using covidhubutils
inc_case_forecast_p6<-load_forecasts(models=c("COVIDhub-ensemble"),forecast_dates =last_6_mondays,locations =state,  types = c("quantile"),targets = c(inc_case_targets),  verbose = FALSE)
...
table1c_cap <- paste0("Table 1c: US national and state observed and predicted COVID-19 cases for the previous week and two weeks ahead.")

table1c_Observed <- paste0("Observed: (", format(last_5_saturdays[4]+1, "%m-%d-%Y") ," to ",format(last_5_saturdays[5], "%m-%d-%Y"),")")

table1c_Predicted <- paste0("Predicted: (", format(last_5_saturdays[5]+8, "%m-%d-%Y") ," to ",format(last_5_saturdays[5]+14, "%m-%d-%Y"),")")
...
datatable(wide_point_dat2p %>%
  ...
  colnames=c('Geography'='location_name.x',
    ...
    'Daily cases per 100K (Pred)'='next_2wk_cases_rate',
...
```{r, child = if (state !='11' & state < '60') 'all-states-weekly-county_code.Rmd'}
