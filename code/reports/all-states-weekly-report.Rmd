...
saturday_2_wk_ahead_txt <- format(saturday_2_wk_ahead, "%B %d")
...
inc_case_targets <- paste(1:2, "wk ahead inc case")
...
state_inc_case_wk_pi_round <- dat %>% 
  filter(fips==state, target == "2 wk ahead inc case", quantile %in% c(0.025, 0.975)) %>% 
  pull(value) %>% sort() %>%
...
As of September 6, 2021 the ensemble forecast for cases only reports up to two-week ahead forecasts, due to persistent large inaccuracies observed when forecasting beyond two weeks. 

For the week ending `r saturday_2_wk_ahead_txt`, the ensemble model predicts that weekly totals of observed cases in each of the next two weeks will be between `r state_inc_case_range[1]` and `r state_inc_case_range[2]` cases each week. However for the week ending `r saturday_4_wk_ahead_txt`, the ensemble shows substantial uncertainty, with observed cases between `r state_inc_case_wk_pi_round[1]` and `r state_inc_case_wk_pi_round[2]` deemed possible (95% prediction interval:  `r state_inc_case_wk_pi[1]` -  `r state_inc_case_wk_pi[2]`).
