...
state_inc_death_wk_pi <- dat %>% 
  ...
  format(big.mark = ",")
```
At the state level, the ensemble model predicts that weekly totals of observed deaths in each of the next four weeks will be between `r state_inc_death_range[1]` and `r state_inc_death_range[2]` deaths each week with around `r state_cum_deaths` total deaths by `r saturday_4_wk_ahead_txt` (Figure 1). However for the week ending `r saturday_4_wk_ahead_txt`, the ensemble shows substantial uncertainty, with observed deaths between `r state_inc_death_wk_pi_round[1]` and  `r state_inc_death_wk_pi_round[2]` deemed possible (95% prediction interval:  `r state_inc_death_wk_pi[1]` -  `r state_inc_death_wk_pi[2]`).
...
## State level
At the state level, the ensemble model predicts that weekly totals of observed cases in each of the next four weeks will be between `r state_inc_case_range[1]` and `r state_inc_case_range[2]` cases each week (Figure 2). However for the week ending `r saturday_4_wk_ahead_txt`, the ensemble shows substantial uncertainty, with observed cases between `r state_inc_case_wk_pi_round[1]` and `r state_inc_case_wk_pi_round[2]` deemed possible (95% prediction interval:  `r state_inc_case_wk_pi[1]` -  `r state_inc_case_wk_pi[2]`).
...
# submit query for forecasts past 6 weeks-using covidhubutils
inc_case_forecast_p6<-load_forecasts(models=c("COVIDhub-ensemble"),forecast_dates =last_6_mondays,locations =state_county,  types = c("quantile"),targets = c(inc_case_targets),  verbose = FALSE)
...
```
For state-level case forecasts made over the last 6 weeks in `r state_name` for which observed data are now available, the 95% prediction intervals covered the truth `r PI95_covern`% of the time, and the 50% prediction intervals covered the truth `r PI50_covern`% of the time. The forecasts on average over the last 6 weeks predicted values that were `r PI50_bias` than the observed data.
...
```{r accuracy of models past 6 weeks-county, eval = state !='11'}
...
```{block eval=FALSE, include = state !='11'}
## County level
...
```{r process-ensemble-data-cases for counties,  eval = state !='11'}
...
```{r make-datatable-inc-case-rates county,  eval = state !='11'}
...
```{r plot-county-cases,  eval = state !='11'}
# create caption
