...
# # use fixed date
# next_saturday <-  as.Date("2021-01-09")
...
models_this_week<-load_forecasts(
  ...
  unique()
# 
# DELETE WHEN DONE OLD CODE
# models_this_week_OLD <- zoltr::do_zoltar_query(
...
# DELETE WHEN DONE OLD CODE
# inc_death_truth_OLD <- read_csv("../../data-truth/truth-Incident Deaths.csv") %>%
...
# DELETE WHEN DONE OLD CODE
# cum_death_start_OLD <- read_csv("../../data-truth/truth-Cumulative Deaths.csv") %>%
...
# DELETE WHEN DONE OLD CODE
# recent_inc_death_totals_OLD<- read_csv("../../data-truth/truth-Incident Deaths.csv") %>%
...
#                     filter(target_end_date == last_5_saturdays[5]) %>%
#                     select(target_end_date, location_name.x,fips, value) %>%
#                     rename(cum_cases_at_forecast_start = value,date =target_end_date,location=fips,location_name=location_name.x)
# 
# 
# cum_case_start_OLD <- read_csv("../../data-truth/truth-Cumulative cases.csv") %>%
#   filter(date == last_5_saturdays[5]) %>%
#   select(date, location, location_name, value) %>%
...
# DELETE WHEN DONE OLD CODE
recent_inc_case_totals_OLD<- read_csv("../../data-truth/truth-Incident Cases.csv") %>%
  # filter(date > last_5_saturdays[3] & date <= last_5_saturdays[5]) %>%
...
# DELETE WHEN DONE OLD CODE
# inc_case_truth_OLD <- read_csv("../../data-truth/truth-Incident Cases.csv") %>%
#   rename(target_end_date = date, fips = location) %>%
#   mutate(model = "observed data (JHU)") %>%
#   group_by(fips, model) %>% arrange(target_end_date) %>%
#   mutate(value = RcppRoll::roll_sum(value, 7, align = "right", fill = NA)) %>%
