...
# author: "Nicholas G Reich, Estee Y Cramer, Martha W Zorn <br><br>https://covid19forecasthub.org/"  (A)
author: "Nicholas G Reich, Estee Y Cramer, Martha W Zorn | https://covid19forecasthub.org/"
...
At the national level, the ensemble model's best guess is that we will see between `r us_inc_death_range[1]` and `r us_inc_death_range[2]` deaths each week for the next four weeks (Figure 1) with around `r us_cum_deaths` deaths by `r saturday_4_wk_ahead_txt` (95% prediction interval: `r us_cum_deaths_wk_pi[2]` - `r us_cum_deaths_wk_pi[1]`). 
...
cum_death_start  <- load_truth(
                    ...
                    left_join(locs, by=c("fips")) %>%
                    filter(target_end_date == last_5_saturdays[5]) %>%
                    select(target_end_date, location_name,fips, value) %>%
                    rename(cum_deaths_at_forecast_start = value,date =target_end_date,location=fips) 
