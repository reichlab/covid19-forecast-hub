...
```{r get-date-boundaries}
 # use fixed date
next_saturday <-  as.Date("2021-03-06")
...
us_inc_case_wk_pi_round <- dat %>% 
  filter(fips=="US", target == "4 wk ahead inc case", quantile %in% c(0.025, 0.975)) %>% 
  pull(value) %>% sort() %>%
  round(-2) %>% format(big.mark = ",")
...
us_inc_hosp_wk_pi_round <- dat_hosp %>% 
  filter(fips=="US", target == "28 day ahead inc hosp", quantile %in% c(0.025, 0.975)) %>% 
  pull(value) %>% sort() %>%
  round(-2) %>% format(big.mark = ",")
...
At the national level, the ensemble model predicts that weekly totals of observed deaths in each of the next four weeks will be between `r us_inc_death_range[1]` and `r us_inc_death_range[2]` deaths with around `r us_cum_deaths` deaths by `r saturday_4_wk_ahead_txt` (95% prediction interval: `r us_cum_deaths_wk_pi[2]` - `r us_cum_deaths_wk_pi[1]`). 

For the week ending `r saturday_4_wk_ahead_txt`, the ensemble forecasts that reported COVID-19 deaths in the US will be between `r us_inc_death_wk_pi_round[1]` and `r us_inc_death_wk_pi_round[2]` (95% prediction interval: `r us_inc_death_wk_pi[1]` - `r us_inc_death_wk_pi[2]`) and COVID-19 cases in the US will be between `r us_inc_case_wk_pi_round[1]` and `r us_inc_case_wk_pi_round[2]` (95% prediction interval: `r us_inc_case_wk_pi[1]` - `r us_inc_case_wk_pi[2]`). 

For `r format(wk4_monday, "%B %d, %Y")` COVID-19 daily hospitalizations in the US will be between `r us_inc_hosp_wk_pi_round[1]` and `r us_inc_hosp_wk_pi_round[2]` (95% prediction interval: `r us_inc_hosp_wk_pi[1]` - `r us_inc_hosp_wk_pi[2]`).
...
mutate(model = "observed data (JHU)") %>%     #maybe also deleting this line
                    left_join(locs, by=c("fips")) %>%
                    filter(fips %in% unique(inc_case_forecast$fips)) 
...
```{r make-US-inc-hospitalization-plot - rolling average}
# 
# 
# 
# #calculate rolling 7 day average
# inc_hosp_truth_7ave <- inc_hosp_truth %>%
#     dplyr::arrange(desc(fips)) %>% 
#     dplyr::group_by(fips) %>% 
#     dplyr::mutate (hosp_07da = zoo::rollmean(value,k=7,fill=NA)) %>% 
#  dplyr::ungroup() %>%    
#  select(model,target_variable,target_end_date,fips,location_name.x,hosp_07da) %>%
#  rename (value=hosp_07da)
#   
# 
# inc_hosp_all_points_7ave <- inc_hosp_truth_7ave %>%
#   bind_rows(filter(inc_hosp_forecast, class=="point")) %>%
#   bind_rows(filter(inc_hosp_truth_7ave, target_end_date==last_5_saturdays[5]) %>% mutate(model="COVIDhub-ensemble")) %>%
#   mutate(model = relevel(factor(model), ref="Observed Data (HealthData)"))
#   
# 
# 
...
#       ymin=lower, ymax=upper,
#       fill=`Prediction Interval`)) +
#   geom_line(data=inc_hosp_all_points_7ave %>%
#       dplyr::filter(fips == "US"),
#     mapping = aes(x = target_end_date, y = value, color = model)) +
#   geom_point(data=inc_hosp_all_points_7ave %>%
#       dplyr::filter(fips == "US", !(model=="COVIDhub-ensemble" & target_end_date <= this_monday)),
#     mapping = aes(x = target_end_date, y = value, color = model)) +
#   scale_fill_manual(values = blues[1:(length(blues)-1)]) +
#   scale_color_manual(values = c("black", tail(blues,1))) +
#   scale_x_date(name = NULL, date_breaks="1 month", date_labels = "%b %d") +
#   ylab("incident hospitalizations") +
#   labs(title="7 day rolling average COVID-19 hospitalization in the US: observed and forecasted",
#     caption="source: HealthData (observed data), COVID-19 Forecast Hub (forecasts)") +
#   theme(legend.position = c(.05,.95), legend.justification = c(0,1),legend.box="horizontal")
...
## color for rates
case_rate_max_1wk <- max(c(wide_point_dat2$last_2wk_cases_rate, wide_point_dat2$next_2wk_cases_rate))
...
table2b_cap <- paste0("Table 2b: US national and state observed and predicted COVID-19 cases for the previous week and two weeks ahead.")

table2b_Observed <- paste0("Observed: (", format(last_5_saturdays[4]+1, "%m-%d-%Y") ," to ",format(last_5_saturdays[5], "%m-%d-%Y"),")")

table2b_Predicted <- paste0("Predicted: (", format(last_5_saturdays[5]+8, "%m-%d-%Y") ," to ",format(last_5_saturdays[5]+14, "%m-%d-%Y"),")")
...
## color for rates
hosp_rate_max_1wk <- max(c(wide_point_dat_hosp$last_2wk_hosp_rate, wide_point_dat_hosp$next_2wk_hosp_rate))
