...
ids = rlang::syms(lapply(forecast_files, function(f) unlist(strsplit(as.character(f), "/"))[2]))
ids_times = rlang::syms(lapply(forecast_files, function(f) unlist(strsplit(as.character(f), "/"))[3]))
