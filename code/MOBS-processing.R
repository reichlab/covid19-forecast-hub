...
day_aheads <- tibble(target = paste(1:7, "day ahead cum death"), target_end_date = forecast_date+1:7)
if(wday(forecast_date) <= 2 ) { ## sunday = 1, ..., saturday = 7
    ...
    week_aheads <- tibble(target = paste(1:7, "wk ahead cum death"), target_end_date = get_next_saturday(forecast_date+seq(0, by=7, length.out = 7)))
} else {
    ## if timezero is after Monday, then the next epiweek is "1 week-ahead"
    week_aheads <- tibble(target = paste(0:7, "wk ahead cum death"), target_end_date = get_next_saturday(forecast_date+seq(0, by=7, length.out = 8)))
}
