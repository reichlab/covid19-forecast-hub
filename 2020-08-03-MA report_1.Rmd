---
title: "COVID-19 Massachusetts Forecast Hub report, 2020-08-03 "
...
last_5_saturdays <- as.Date("2020-08-03") - 7*c(5:1)
# this_monday <- next_saturday - 5
this_monday <- as.Date("2020-08-03")
```
...
This past week, 44 models were submitted, including one model from a new team. 
...
dat <- zoltr:: do_zoltar_query(
  ...
  units=c("US","25","25001","25003","25005","25007","25009","25011","25013","25015","25017","25019","25021","25023","25025","25027"),
...
This week, our ensemble combined forecasts from `r n_unique_models` different models. We note that this week several states, most notably Texas, modified the way in which they were reporting "probable" COVID-19 deaths. In Texas, this resulted in [a one-time additional count of over 600 deaths](https://www.dshs.texas.gov/news/releases/2020/20200727.aspx) in the data that we consider to be "ground truth".

At the state level, the ensemble model's best guess is that we will see between 91 and 106 deaths each week for the next four weeks with around 9087 deaths by August 29th (Figure 1). However for the week ending August 29th, the ensemble shows substantial uncertainty, with observed deaths between 4,500 and 10,500 deemed possible (95% prediction interval: 4,560 - 10,587) WHERE DO i FIND THESE NUMBERS?.
...
ggplot() +
  ...
  labs(title="Weekly reported COVID-19 deaths in the Massachusetts: observed and forecasted",
...
recent_inc_case_totals <- read_csv("../../data-truth/truth-Incident Cases.csv") %>%
  ...
  select(date, location, location_name, value, last_2wk, last_4wk) %>%
  group_by(location, location_name) %>%
  summarize(last_2wk_cases = sum(value*last_2wk),
    last_4wk_cases = sum(value*last_4wk)) %>%
  ungroup() %>%
  left_join(locs, by=c("location" = "FIPS")) %>%
...
```{r one-time-tx-fix}
## subtract 631 deaths from Texas total in last week
## https://www.dshs.texas.gov/news/releases/2020/20200727.aspx: 5713 - 5038 - 44

recent_inc_death_totals[which(recent_inc_death_totals$location_name=="Texas"), "last_2wk_deaths"] <- recent_inc_death_totals[which(recent_inc_death_totals$location_name=="Texas"), "last_2wk_deaths"] - 631

recent_inc_death_totals[which(recent_inc_death_totals$location_name=="Texas"), "last_4wk_deaths"] <- recent_inc_death_totals[which(recent_inc_death_totals$location_name=="Texas"), "last_4wk_deaths"] - 631
...
## County Breakdown for incidence of cases

The ensemble model estimates that `r nrow(quant.5_cutoff)` states and territories of the US have a greater than 50% chance of having more deaths in the next two weeks compared to the past two weeks (Table 1). But the model shows substantial uncertainty about the increases, with only `r nrow(quant.25_cutoff)` states and territories showing a greater than 75% chance of an increase over the next two weeks (`r paste(quant.25_cutoff$location_name, collapse=", ")`). Note that for the purposes of the tables below, we did not count any of the 631 "probable" COVID-19 deaths [newly reported last week in Texas](https://www.dshs.texas.gov/news/releases/2020/20200727.aspx) as having occured in the last two weeks.

<!-- In the following states, the probability that the deaths in the next two weeks will greater than in the last two weeks is greater than or equal to 85% -->
<!-- ```{r} -->
<!-- knitr::kable(wide_quant_dat1$location_name, col.names  = NULL) -->
<!-- ``` -->




The sortable and searchable table below shows the total number of reported COVID-19 deaths at the US level and for Massachusetts over the last two weeks (ending Saturday, August 1, 2020) and the forecasted counts for the subsequent two weeks (ending August 15, 2020).
...
datatable(wide_point_dat %>% 
  ...
  caption = 'Table 1: US national and Massachusetts observed deaths to date and for the previous two weeks (ending August 1, 2020) with forecasted COVID-19 deaths for the next two weeks (ending August 15, 2020).',
  options = list(
    autoWidth = TRUE,
...
The sortable and searchable table below shows the total number of reported COVID-19 cases at the county level for Massachusetts as of Saturday, August 1, 2020 ("Total COVID-19 Cases") as well as the rate of reported COVID-19 cases in the population (standardized per 100,000 population) over the last two weeks and over the next two weeks. Looking at the rates allows for easier comparison across counties, where you can see which counties have had or are predicted to have propoportionally higher rates in comparison to other counties. These tables calculate an average daily number of cases per 100,000 population across the last two weeks (ending Saturday, August 1, 2020) and forecasted for the following two weeks (ending August 15, 2020).
...
wide_point_dat1 <- spread(ensemble_pointdat1, target, value) %>%
  ...
  mutate(next_2wk_cases = `2 wk ahead inc case`,
    diff_2wk_cases = next_2wk_cases - last_2wk_cases,
    next_4wk_cases = `4 wk ahead inc case`,
    diff_4wk_cases = next_4wk_cases - last_4wk_cases,
    pop_x_1k = round(Population/1000),
    last_2wk_cases_rate = round(last_2wk_cases/Population*100000/14,3),
    last_4wk_cases_rate = round(last_4wk_cases/Population*100000/28,3),
    next_2wk_cases_rate = round(next_2wk_cases/Population*100000/14,3),
    next_4wk_cases_rate = round(next_4wk_cases/Population*100000/28, 3),
    diff_2wk_cases_rate = round(next_2wk_cases_rate - last_2wk_cases_rate, 3),
    diff_4wk_case_rate = round(next_4wk_cases_rate - last_4wk_cases_rate, 3)) %>%
  select(location_name, Population, cum_cases_at_forecast_start,
    last_2wk_cases, next_2wk_cases, diff_2wk_cases,
    last_4wk_cases, next_4wk_cases, diff_4wk_cases,
    last_2wk_cases_rate, next_2wk_cases_rate,
    last_4wk_cases_rate, next_4wk_cases_rate,
    diff_2wk_cases_rate, diff_4wk_case_rate)
...
## color for rates
case_rate_max_2wk <- max(c(wide_point_dat1$last_2wk_cases_rate, wide_point_dat1$next_2wk_cases_rate))
brks <- seq(0, case_rate_max_2wk, length.out = 100) #quantile(df, probs = seq(.05, .95, .05), na.rm = TRUE)
...
datatable(wide_point_dat1 %>% 
    select(location_name, Population,
      ...
      last_2wk_cases_rate, next_2wk_cases_rate, diff_2wk_cases_rate) %>%
    arrange(desc(diff_2wk_cases_rate)),
  caption = 'Table 2: County level observed and predicted daily case rates for the previous two weeks (ending August 1, 2020) and the next two weeks (ending August 15, 2020).',
  options = list(
    autoWidth = TRUE,
    columnDefs = list(list(width = '100px', targets = c(0, 1, 2, 3, 4, 5)))
  ),  #width=paste0(c(10, 100, 100, 100), 'px'),
  ...
  colnames=c('county'='location_name', 
    ...
    'Daily cases per 100k, last 2 weeks'='last_2wk_cases_rate',
    'Daily cases per 100k, next 2 weeks'='next_2wk_cases_rate',
    'Case rate difference' = 'diff_2wk_cases_rate')) %>%
