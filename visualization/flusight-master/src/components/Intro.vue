...
export default {
  ...
  ready() {
    ...
    this.appendIntroItems([
      ...
      {
        title: "State",
        content: `Use this drop down to select the state to show predictions
                  for.`,
        direction: "right",
      ...
      {
        title: "US Map",
        content: `<p>The map shows observed cumulative deaths for the currently selected week.</p>
      ...
      },
      {
        title: "Finished",
        content:
          `Check out the source for this app and provide feedback on
                  the project's github page <a href="` +
          this.branding.sourceUrl +
  ...
  watch: {
    introStep: function() {
      ...
      if (this.lastElement !== "") {
        d3.select(this.lastElement).style("z-index", null);
      }
      this.demoStep(this.currentIntro);
